...
AlienFlightNG（下一代）设计以 CC BY-NC-SA 4.0 或 CC BY-NC-ND 4.0 许可用于非商业用途，详见：
...
- MCU：STM32F405RGT6（ALIENFLIGHTF4）
...
- 集成 USB 接口
- 提供外接 DSM2/DSMX 卫星接收机（如 Spektrum SAT、OrangeRx R100、Lemon RX、Deltang Rx31）和 SBUS 的串行接口
- CPPM 输入
...
- 电流监测（F4 V1.1）
- SD 卡读卡器，用于 Blackbox 记录（F4 V1.1）
- 集成 OpenSky（兼容 FrSky）接收机，并支持 FrSky Hub 遥测（F4/F7 V2）
...
AlienFlight 固件目标包括 ALIENFLIGHTF1、ALIENFLIGHTF3、ALIENFLIGHTF4 和 ALIENFLIGHTNGF7。固件附带适配的默认设置，可实现即插即用。使用小型四轴飞行器无需计算机即可起飞。默认设置包含八轴飞行器的自定义混控器，可实现整洁直连布线；在 CLI 中执行 `mixer custom` 即可启用。若用于六轴或八轴飞行器，或需要进一步调参，可在 CLI 或 Betaflight App 中继续配置。
