...
在 Betaflight CLI 中输入 `help`，即可显示可用命令及其用法。
...
输入 `get`，再加上 CLI 参数名称的一部分（例如 `get acc`）。CLI 会列出名称包含该片段的所有参数，并显示其当前值以及有效值范围或可用名称。借此可以快速定位变量名，并从 CLI 输出复制后粘贴到命令行输入框中。

使用 `set` 命令加上参数名和值即可修改配置。修改后务必执行 `save` 命令。
